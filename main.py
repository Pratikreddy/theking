...
def openai_call(messages, model, system_message):
...
def the_king(king_model, peasant_models, user_message):
    ...
    for i, model in enumerate(peasant_models):
        ...
        if model in gpt_models:
            answers[f"Peasant {i+1} ({model})"] = openai_call(user_message, model, "You are a coder and problem solver expert")
        else:
            answers[f"Peasant {i+1} ({model})"] = groq_call(user_message, model)
        progress_bar.update()
    ...
    if king_model in gpt_models:
        king_answer = openai_call(king_prompt, king_model, "You are a wise and knowledgeable coder and problem solver king who provides thoughtful answers to questions.")
    else:
        king_answer = groq_call(king_prompt, king_model)
...
if st.button("Solve Problem"):
    ...
    elif not (openai_client and groq_client):
